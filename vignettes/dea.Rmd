...
metadata_genes <- filter(annotation, !duplicated(ensembl_gene_id)) %>% left_join(gene_count_raw, ., by = "ensembl_gene_id") %>%
...
dea_pretty <- dea_raw %>%
    ...
    rownames_to_column("ensembl_gene_id") %>%
      ...
      format_dea(fc_threshold = fc_threshold, p = p_threshold) %>%
...
```{r, eval = FALSE}
(top_description <- top_genes %>% pull(1) %>% map(~get_ncbi_description(., annotation)) %>% list.rbind())
```
...
# Heatmap
```{r}
# filter(metadata_genes, gene_name %in% c("DDX3Y", "XIST")) %>% plot_heatmap()
```
