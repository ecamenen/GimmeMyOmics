---
title: "Differential expression analysis"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Differential expression analysis}
  %\VignetteEngine{knitr::rmarkdown}
  %\VignetteEncoding{UTF-8}
---

```{r, include = FALSE}
knitr::opts_chunk$set(
  collapse = TRUE,
  comment = "#>"
)
```

```{r setup}
set.seed(123)
library("GimmeMyOmics")
library("GimmeMyPlot")
library("DESeq2")
library("pheatmap")
library("magrittr")
library("RColorBrewer")
# source("R/utils_theme.R")
# source("R/utils_bulk.R")
genome_species <- "hsapiens"
# genome_species <- "mmusculus"
genome_version <- 113
fc_threshold <- 2
fc_threshold <- log2(fc_threshold)
p_threshold <- 0.05
```

```{r}
data("tnbc_neutrophils")
(gene_count_raw <- tnbc_neutrophils$gene_counts)
metadata_samples <- tnbc_neutrophils$metadata %>%
        as.data.frame() %>%
        mutate(across(everything(), as.factor)) %>%
        column_to_rownames("id")
metadata_samples$condition <- relevel(metadata_samples$condition, ref = "HD")

data("biomart_annotation")
(annotation <- biomart_annotation[[paste(genome_species, genome_version, sep = "_")]])

name_id <- "ensembl_gene_id"
# name_id <- "gene_name"
metadata_genes <- filter(annotation, !duplicated(ensembl_gene_id)) %>% 
    left_join(gene_count_raw, ., by = name_id) %>%
    select(colnames(annotation))
```

```{r}
pull(metadata_genes, "chromosome_name") %>% table()
pull(metadata_genes, "gene_biotype") %>% 
    fct_count() %>% 
    arrange(desc(n)) %>% 
    print(n = 50)
```

# Data preparation
```{r}
gene_count <- gene_count_raw %>%
  as.data.frame() %>%
  column_to_rownames(name_id) %>%
  round()

all(rownames(metadata_samples) == colnames(gene_count))
```

```{r}
 dea_df <- DESeqDataSetFromMatrix(
  countData = gene_count,
  colData = metadata_samples,
  design = ~ condition
) %>%
   .[rowSums(counts(.) >= 10) >= 3, ]

model.matrix(dea_df@design, data = metadata_samples) %>% as.data.frame()
nrow(gene_count)
nrow(counts(dea_df))
nrow(counts(dea_df)) / nrow(gene_count) * 100
```

# DEA
```{r, message = FALSE, warning=FALSE}
dea <- DESeq(dea_df, quiet = TRUE)
resultsNames(dea)
sizeFactors(dea)
# use_data(dea, overwrite = TRUE)
# coef(dea) %>% kable0() %>% head()
# saveRDS(dea, file = file.path(path, paste0("dea", project_name, ".RDS")))
# dea <- readRDS(file.path(path, paste0("dea", project_name, ".RDS")))
```

```{r}
plotDispEsts(dea)
boxplot(log10(assays(dea)[["cooks"]]), range=0, las=2)
```

# PCA plot
```{r, message = FALSE}
count_normalized <- vst(dea, blind = FALSE)
conditions <- dea@design %>%
  as.character() %>%
  .[2] %>% 
  str_split("[+:]") %>%
  unlist() %>%
  str_trim() %>%
  unique()


pca_data <- plotPCA(count_normalized, intgroup = conditions[-3], returnData = TRUE)

rv <- rowVars(assay(count_normalized))
select <- order(rv, decreasing = TRUE)[seq_len(500)]
pca <- prcomp(t(assay(count_normalized)[select, ]))

percentVar <- pca$sdev^2 / sum(pca$sdev^2)

centroids <- pca_data %>%
  group_by(group) %>%
  summarise(PC1 = mean(PC1), PC2 = mean(PC2))

ggplot(pca_data, aes(PC1, PC2, color = group)) +
  geom_point(size = 3) +
  stat_ellipse() +
  geom_point(
    data = centroids,
    aes(PC1, PC2, color = group),
    shape = 4, size = 5, stroke = 1.5
  ) +
  labs(
    x = paste0("PC1: ", round(percentVar[1] * 100), "% variance"),
    y = paste0("PC2: ", round(percentVar[2] * 100), "% variance")
  ) +
  theme_custom() +
  scale_color_discrete(type = palette_discrete())

```

```{r}
sampleDists <- dist(t(assay(count_normalized)))
pheatmap(
  as.matrix(sampleDists) %>%
  set_rownames(pull(metadata_samples, 1)),
  clustering_distance_rows = sampleDists,
  clustering_distance_cols = sampleDists,
  col = colorRampPalette(rev(brewer.pal(9, "Reds")))(255)
)
```

# Normalized plot
```{r, eval = FALSE}
library(vsn)
meanSdPlot(assay(count_normalized))
```

```{r, eval = FALSE}
glimmaMDS(dea)
glimmaMA(dea)
```

# Contrasts
```{r}
# dea_raw <- results(dea, alpha = 1-1e-10) %>%
dea_raw <- lfcShrink(
    dea,
    contrast = c(colnames(metadata_samples)[1], levels(pull(metadata_samples, 1))),
    type = "ashr",
    quiet = TRUE
)
summary(dea_raw)

# saveRDS(dea_raw, file = file.path(path, paste0("dea_contrast", project_name, ".RDS")))
```

```{r}
dea_pretty <- format_dea(
    dea_raw,
    metadata_genes, 
    fc_threshold = fc_threshold, 
    p_threshold = p_threshold,
    name_id = name_id,
    dea = dea
)

table(pull(dea_pretty, "Expression")) %>% 
  as.data.frame() %>% 
  kable0()
```

# Volcano plot
```{r, message = FALSE, warning = FALSE, fig.width=7, fig.height=5}
top_genes <- top_genes(
    dea_pretty,
     fc_threshold = fc_threshold,
     p_threshold = p_threshold,
     n = 15
)
volcano_plot(
  dea_pretty,
  top_genes = top_genes,
  fc_threshold = fc_threshold,
  p_threshold = p_threshold
)
```

```{r, eval = FALSE}
library("rvest")
(top_description <- top_genes %>% 
     pull(1) %>% 
     map(~ncbi_description(., annotation)) %>%
     list.rbind()
)
```


```{r, message=FALSE, warning=FALSE, eval = FALSE}
plot_counts <- function(x, name_id = "ensembl_gene_id") {
  list_cbind0 <- function(x) {
  row_names <- lapply(x, rownames)
  x <- map(x, ~ as.data.frame(.))
  tmp <- sapply(x, nrow)
  for (i in seq(length(tmp))) {
    add <- NULL
    add0 <- NULL
    if(i > 1) {
      add <- matrix(nrow = sum(tmp[1:(i-1)]), ncol = ncol(x[[i]]))
      if (is.data.frame(x[[i]])) {
        add <- as.data.frame(add)
      }
      colnames(add) <- colnames(x[[i]])
    }
    if(i < length(tmp)) {
      add0 <- matrix(nrow = sum(tmp[(i+1):length(tmp)]), ncol = ncol(x[[i]]))
      if (is.data.frame(x[[i]])) {
        add0 <- as.data.frame(add0)
      }
      colnames(add0) <- colnames(x[[i]])
    }
      x[[i]] <- rbind(add, x[[i]], add0)
      rownames(x[[i]]) <- unlist(row_names)
  }
  list.cbind(x)
}

  
df <- plotCounts(
    dea,
    pull(x, name_id),
    intgroup = conditions[-3],
    returnData = TRUE
)

 map(
        pull(df, "condition") %>% unique(),
        ~ filter(df, condition == .) %>% select(count)
    ) %>%
    list_cbind0() %>%
    set_colnames(pull(df, "condition") %>% unique()) %>% 
    plot_violin(pch_size = 4, display_log = TRUE, title = pull(x, "gene_name"))
}

top_genes[seq(3), "description"]

map(seq(3), ~plot_counts(top_genes[.,], name_id = name_id)) -> pp
pp[[1]] + pp[[2]] + pp[[3]]
```

```{r}
(table_dea <- print_dea(dea_pretty))
# write_delim(table_dea, "dea.csv", delim = ";", col_names = TRUE)
```

# Heatmap
```{r}
# filter(metadata_genes, gene_name %in% c("DDX3Y", "XIST")) %>% 
#     plot_heatmap(metadata_samples, count_normalized)
```

```{r}
plot_heatmap(top_genes, metadata_samples, count_normalized)
```

```{r}
sessionInfo()
```
